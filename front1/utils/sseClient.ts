...
export interface SSEEvent {
  type: 'token' | 'tool_start' | 'tool_end' | 'tool_error' | 'workflow' | 'intent' | 'plan' | 'step_start' | 'step_end' | 'step_error' | 'retrieval' | 'finish' | 'error';
  data: any;
}
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/chat_stream`, {
      ...
      body: JSON.stringify({
        ...
        top_k_override: options?.topKOverride,
      }),
    });
