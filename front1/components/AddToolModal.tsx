...
interface AddToolModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (name: string, description: string, introduction: string, config: string) => Promise<void>;
  onTestConnection: (toolName: string, description: string, config: string) => Promise<{ success: boolean; message: string }>;
}

export function AddToolModal({ isOpen, onClose, onAdd, onTestConnection }: AddToolModalProps) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      JSON.parse(config || defaultConfigTemplate);
      const result = await onTestConnection(name.trim(), description, config || defaultConfigTemplate);
  ...
  const handleAdd = async () => {
    ...
    setIsAdding(true);
    try {
      await onAdd(name.trim(), description, description, config || defaultConfigTemplate);
      setIsAdding(false);
      setIsAdded(true);
      ...
      setIsAdding(false);
      toast.error('保存失败', {
        description: error instanceof Error ? error.message : '无法保存工具配置',
        duration: 4000,
      });
    }
  };
