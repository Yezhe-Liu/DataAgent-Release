...
export function LeftSidebar({
  ...
  const formatTime = (timestamp: number) => {
    ...
    if (days === 0) {
      ...
      return new Date(normalizedTimestamp).toLocaleDateString('zh-CN');
    }
  };
  ...
  const startEditing = (conv: Conversation, e: MouseEvent) => {
    e.stopPropagation();
  ...
  const handleDelete = (id: string, title: string, e: MouseEvent) => {
    e.stopPropagation();
