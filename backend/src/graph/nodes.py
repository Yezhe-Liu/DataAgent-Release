...
每个节点采用工厂函数模式，通过闭包注入外部依赖（模型、检索器、工具等），
实现 graph 模块与 src.tools / src.rag_engine 的解耦。

节点职责:
  router             - 意图分类 (chat / rag / tool)
  rewrite            - Multi-Query 多角度查询重写
  retrieve           - 知识库多路检索 + 合并去重
  grade              - LLM 文档相关性评分
  web_search         - 外网搜索回退
  tool_execute       - 数据分析/绘图工具调用 (mini ReAct)
  generate           - 基于检索结果生成含引用回答
...
- chat: 用户在进行闲聊、问候、自我介绍等无需检索或计算的对话
- rag:  用户询问的知识需要从企业知识库中检索（如政策、流程、规范、FAQ 等）
- tool: 用户需要执行 Python 数据分析或生成图表（如"分析销售数据"、"画散点图"）
...
# =============================================================================
# 7. Generate — 基于检索结果生成回答
# =============================================================================

GENERATE_SYSTEM = """你是一个企业知识库问答助手 DataAgent。基于以下检索到的文档片段回答用户问题。

要求:
1. 先给出结论，再展开细节
2. 引用文档时必须标注来源编号，如 [1]、[2]
3. 如果检索结果中没有完整答案，诚实说明并给出已知信息
4. 不要编造检索结果中没有的信息
